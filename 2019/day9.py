...
def main(program, input_value):
    logging.info('STARTING: [%s] with input value: [%s]', program.name, input_value)
    running = True
    while running:
        for out in program.runner(input_value):
            if out is None:
                running = False
                break
            res = out
    logging.info('FINISHED: [%s] with final output value: [%s]', program.name, res)
